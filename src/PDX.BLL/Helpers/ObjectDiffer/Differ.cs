...
namespace PDX.BLL.Helpers.ObjectDiffer
{
    public class Differ: IDiffer
    {
        ...
        private Difference GetDifferences(object newObj, object oldObj, string propName, Type objectType, params string[] ignoreList)
        {
            ...
            return _typeDiffers.First(d => d.CanPerformDiff(objectType)).PerformDiff(newObj, oldObj, propName, objectType, GetDifferences, ignoreList);
        }

        // just a wrapper for the GetDifferences method to make the public API a bit nicer
        public Difference Diff<T>(T newObj, T oldObj, params string[] ignoreList)
        {
            return GetDifferences(newObj, oldObj, "self", typeof(T), ignoreList);
        }
    }
}